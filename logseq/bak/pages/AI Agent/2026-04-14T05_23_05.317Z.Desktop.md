...
- # How to Build
	...
	- ## Harness
		- ### Guidance
			...
			- **Agent roles**: separate agents by concern — initializer (setup), coder (incremental progress), planner (expand specs), evaluator (validate output). Single generalist agents consistently underperform multi-agent systems with clear role boundaries. Evaluators overcome self-praise bias. Agent-to-agent review can replace most human review over time.
			- **Documentation as infrastructure**: keep AGENT.md short (~100 lines) as a table of contents, not an encyclopedia — a giant file crowds out the task, rots instantly, and becomes non-guidance. Knowledge base lives in structured `docs/` directory as system of record. Enforce freshness mechanically with linters and "doc-gardening" agents. Anything not in the repo effectively doesn't exist for the agent (Slack threads, Google Docs, tacit knowledge).
			...
			- **Architecture**: agents work best in rigid, predictable structures — this is an early prerequisite, not a late optimization. Enforce strict boundaries (dependency directions, layer separation) via custom linters and structural tests. Prefer "boring" technologies — composable, stable APIs, well-represented in training data. Write custom lint error messages that inject remediation instructions into agent context. Enforce invariants (e.g. validate at boundaries), not implementations.
			- **Application legibility**: make the app itself legible to agents — bootable per worktree, UI driveable via Chrome DevTools/Puppeteer MCP (DOM snapshots, screenshots, navigation), logs/metrics/traces queryable via local observability stack (LogQL, PromQL). This lets agents reproduce bugs, validate fixes, and reason about runtime behavior directly.
			- **Testing & quality gates**: test immediately after each unit. Use browser automation for end-to-end verification — agents identify and fix bugs not obvious from code alone. Use static analysis for dynamic languages. Set minimum thresholds per quality dimension; if any falls below, the sprint fails with detailed feedback. Never let agents remove or edit tests.
			- **Context management**: compaction preserves continuity but doesn't give a clean slate — models can develop "context anxiety" and prematurely wrap up. Context resets need good handoff artifacts. Use git history + progress logs to supplement; they provide clarity compaction cannot guarantee. Budget ~170k tokens per loop.
			...
			- **Evaluation loops**: for creative/subjective work, use generator + evaluator agent pairs with explicit grading criteria. 5-15 iterations per generation, improvement plateaus but middle iterations can beat final ones. Tune evaluators by reading their logs and fixing where judgment diverged from expectations.
			- **Prevent drift & entropy**: explicitly instruct "search before creating", "no placeholders", "full implementations". Agents replicate existing patterns — even bad ones. Encode "golden principles" and run recurring cleanup agents as garbage collection. Technical debt compounds; pay it down continuously in small increments, not painful bursts. Use feature list with pass/fail status to prevent premature completion.
			- **Recovery**: expect broken states. Options: `git reset --hard`, rescue prompts, or switch models. When agent struggles, treat it as a signal — identify what capability is missing (tools, guardrails, docs) and feed it back into the repo.
			- **Human role**: humans steer, agents execute. ~90% automation, 10% senior judgment. Operator owns spec accuracy. Agents produce everything (code, tests, CI, docs, tooling, dashboards, review comments, scripts that manage the repo itself). When something fails, the fix is almost never "try harder" — ask "what capability is missing?"
			- **Evolving harnesses**: every harness component encodes an assumption about what the model can't do. Stress-test assumptions — remove one component at a time and measure impact. As models improve, the boundary moves outward. Human taste is captured once, then enforced continuously on every line of code.
		- ### References & Notes
